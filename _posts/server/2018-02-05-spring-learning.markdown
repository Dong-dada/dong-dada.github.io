...
这一小节介绍如何编写切点来定位应该在什么地方应用切面的通知。

Spring AOP 使用 AspectJ 的切点表达式语言来定义切点。Spring AOP 只支持一部分 AspectJ 切点指示器，如下：
...
上述指示器中，只有 `execution()` 是实际执行匹配的，其它指示器都是用来限制匹配的。也就是说我们首先用 execution 来执行匹配，然后用其它指示器来限制所要匹配的连接点有哪些；
